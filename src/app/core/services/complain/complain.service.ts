...
import { Complain, ComplaintRequest } from '@belisada-seller/core/models';
...
constructor(private configuration: Configuration, private http: HttpClient) { }
    ...
    acceptComplain(data: ComplaintRequest): Observable<ComplaintRequest> {
        return this.http.post(this.configuration.apiURL + '/complain/accept', data)
        .pipe(
            map(response => response as ComplaintRequest)
        );
    }

    rejectComplain(data: ComplaintRequest): Observable<ComplaintRequest> {
